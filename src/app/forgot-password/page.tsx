  ...
  return (
    <>
      <style>{`
        ...
        .a-success{display:flex;flex-direction:column;gap:16px;margin-top:20px;padding:20px;border:1px solid rgba(226,179,110,0.3);background:rgba(226,179,110,0.04);animation:aIn 0.5s cubic-bezier(0.22,1,0.36,1) forwards}
        @keyframes aIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
        .a-success-title{font-size:12px;letter-spacing:0.1em;text-transform:uppercase;color:#e2b36e;font-weight:500;display:flex;align-items:center;gap:8px}
        .a-success-sub{font-size:11px;line-height:1.6;color:rgba(255,255,255,0.7)}
        .a-success-sub strong{color:#ffffff;font-weight:400}
      `}</style>
      ...
      <div className="a-root">
        ...
        <div className={`a-wrap${mounted ? ' ready' : ''}`}>
          <div className="a-eyebrow">Recuperare cont</div>
          <h1 className="a-title">Resetare <em>parolă.</em></h1>
          ...
          {!sent ? (
            <form onSubmit={handleReset} className="a-form">
              <div className="a-field">
                <input 
                  ...
                  required 
                />
                <label htmlFor="f-email" className="a-label">Adresă email</label>
              </div>
              ...
              <div className="a-cta-row">
                <button type="submit" className="a-btn-submit" disabled={loading}>
                  <span>{loading ? 'Se trimite...' : 'Trimite link resetare'}</span>
            ...
            <div className="a-success">
              <div className="a-success-title">
                <span>✉</span> Email trimis cu succes
              </div>
              <div className="a-success-sub">
                Verifică adresa <strong>{email}</strong> pentru a accesa link-ul de securitate și a configura noua parolă.
              </div>
            </div>
          )}
