    ...
    <>
      ...
      <div className="min-h-screen bg-[#faf8f4] arhi-mono relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="pt-16 md:pt-24 pb-24 md:pb-8 px-4">
            <div className="max-w-2xl mx-auto h-[78dvh] md:h-[80vh] flex flex-col">
              ...
              {checkingAuth ? (
                ...
                <>
                  ...
                  <header className="shrink-0 flex items-center justify-between pb-4 mb-2 border-b border-black/[0.06]">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="flex flex-col leading-tight">
                        <span className="text-[13px] font-medium text-black tracking-wide">Arhi AI</span>
                    ...
                    <button
                      onClick={() => setMessages([])}
                      className="group flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider text-zinc-500 hover:text-[#bfa054] transition-colors duration-300"
                    >
                      <RotateCcw size={12} className="transition-transform duration-500 ease-out group-hover:-rotate-180" />
                      Conversație nouă
                    </button>
                  </header>

                  {/* Mesajele stau direct pe pagină, fără cutie/card în jur */}
                  <div ref={scrollContainerRef} className="arhi-scroll flex-1 overflow-y-auto w-full">
                    {messages.length === 0 ? (
                      <div className="h-full flex flex-col justify-center items-center px-2 text-center">
                      ...
                      <div className="pb-4">
                        {messages.map((m) => (
                          <div key={m.id} className="anim-fadeup w-full py-4">
                              ...
                              <div
                                className={`max-w-[82%] px-4 py-2.5 text-[14px] leading-relaxed whitespace-pre-wrap break-words ${
                                  m.role === 'user'
                                    ...
                                    : 'bg-black/[0.04] text-zinc-800 rounded-2xl rounded-bl-md'
                                }`}
                              >
                                {m.content ? (
                                  m.content
                  ...
                  {/* Input — pastilă plutitoare, fără chenar gros, doar umbră discretă */}
                  <div className="shrink-0 pt-4">
                    <div className="relative flex items-end w-full rounded-[26px] bg-white/80 backdrop-blur-md shadow-[0_8px_30px_rgba(26,26,26,0.08)] transition-all duration-300 focus-within:shadow-[0_8px_30px_rgba(191,160,84,0.18)]">
                      <textarea
                        ref={inputRef}
                        rows={1}
                        value={textInput}
                        ...
                        disabled={isLoading}
                        className="flex-1 w-full max-h-[160px] min-h-[50px] py-4 pl-5 pr-12 bg-transparent text-[14.5px] text-black placeholder-zinc-400 resize-none focus:outline-none arhi-mono"
                      />
                      <button
                        onClick={() => { if (textInput.trim()) { executeChat(textInput); setTextInput(''); } }}
                        disabled={isLoading || !textInput.trim()}
                        aria-label="Trimite"
                        className="absolute right-2.5 bottom-2.5 w-8 h-8 rounded-full bg-black text-white flex items-center justify-center disabled:bg-zinc-200 disabled:text-zinc-400 transition-all duration-300 hover:scale-110 hover:bg-[#bfa054] active:scale-95 disabled:hover:scale-100 disabled:hover:bg-zinc-200"
                      >
