...
export default function ArhiChat() {
  ...
  // Autentificare — chat-ul e disponibil doar userilor logați
  ...
  // Scroll stabil: smooth DOAR când apare un mesaj nou (lungimea array-ului crește).
  // În timpul streaming-ului (conținutul unui mesaj existent se tot schimbă),
  // scrollăm instant — altfel animațiile 'smooth' se suprapun la fiecare
  // bucățică de text primită și dau senzația că sare/fuge în sus.
  useEffect(() => {
  ...
  return (
    <>
      <style>{`
        ...
        /* Fundalul "respiră" — zoom lent, continuu, dus-întors */
        @keyframes bgBreathe {
        ...
        .arhi-scroll { scrollbar-width: thin; scrollbar-color: rgba(245,158,11,0.3) transparent; overscroll-behavior: contain; }
        ...
        .arhi-card-frame::before {
          ...
          position: absolute;
          inset: -50%;
          background: conic-gradient(from 0deg, transparent 0%, rgba(245,158,11,0.55) 8%, transparent 22%);
      ...
      <div className="min-h-screen bg-[#0d0d0d] arhi-mono relative overflow-hidden">
        ...
        {/* Fundal real: imaginea, pe toată pagina, cu zoom lent + overlay pentru lizibilitate */}
        <div className="fixed inset-0 z-0 pointer-events-none select-none overflow-hidden">
          <img
            src="/nimet.webp"
            alt=""
            className="anim-bg-breathe w-full h-full object-cover opacity-[0.5]"
          />
          {/* Overlay-uri de gradient — păstrează textul și card-ul lizibile peste imagine */}
          <div className="absolute inset-0 bg-gradient-to-b from-[#0d0d0d]/80 via-[#0d0d0d]/55 to-[#0d0d0d]/90" />
          <div className="absolute inset-0 bg-gradient-to-r from-[#0d0d0d]/70 via-transparent to-[#0d0d0d]/70" />
          {/* Nuanță amber discretă, ca imaginea să se lege de brand, nu doar să stea acolo */}
          <div className="absolute inset-0 bg-amber-900/10 mix-blend-overlay" />
        </div>

        {/* Fundal ambiental — două pete de lumină amber care respiră lent, peste imagine */}
        <div className="fixed inset-0 z-[1] pointer-events-none overflow-hidden">
          <div
            className="absolute top-[8%] left-[10%] w-[420px] h-[420px] rounded-full blur-[110px]"
            style={{ background: 'radial-gradient(circle, rgba(245,158,11,0.18) 0%, transparent 70%)', animation: 'blobFloatA 14s ease-in-out infinite' }}
          />
          <div
            className="absolute bottom-[10%] right-[8%] w-[380px] h-[380px] rounded-full blur-[110px]"
            style={{ background: 'radial-gradient(circle, rgba(245,158,11,0.14) 0%, transparent 70%)', animation: 'blobFloatB 17s ease-in-out infinite' }}
          />
        </div>
        ...
        <div className="relative z-10">
          ...
          <div className="relative pt-20 md:pt-32 pb-28 md:pb-20 px-4">
            <div className="anim-card arhi-card-frame max-w-3xl mx-auto h-[74dvh] md:h-[76vh] shadow-[0_20px_60px_rgba(0,0,0,0.55)]">
              <div className="relative h-full flex flex-col rounded-2xl bg-[#141414]/80 backdrop-blur-xl overflow-hidden">

                {checkingAuth ? (
                  // ── Stare de verificare sesiune ─────────────────────────
                  <div className="h-full flex flex-col items-center justify-center gap-3">
                    <div
                      className="w-6 h-6 rounded-full border-2 border-amber-500/25 border-t-amber-500"
                      style={{ animation: 'spin 0.8s linear infinite' }}
                    />
                    <span className="text-[11px] uppercase tracking-[0.15em] text-neutral-500">Se verifică accesul...</span>
                  </div>
                ) : !userId ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center px-8">
                    ...
                    <h2 className="arhi-serif text-2xl md:text-3xl text-white mb-2">
                    ...
                    <Link
                      href="/login"
                      className="inline-flex items-center justify-center bg-amber-500 text-black rounded-full px-7 py-3 text-[11px] font-bold tracking-[0.2em] uppercase transition-all duration-300 hover:bg-amber-400 active:scale-95"
                    >
                      Autentifică-te
                    </Link>
                  </div>
                  ...
                  <>
                    ...
                    <header className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-white/5 bg-neutral-900/50 backdrop-blur-md">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-neutral-100 tracking-wide">Arhi AI</span>
                    ...
                    <div ref={scrollContainerRef} className="arhi-scroll flex-1 overflow-y-auto w-full relative">
                      {messages.length === 0 ? (
                        <div className="h-full flex flex-col justify-center items-center px-6 text-center">
                          ...
                          <p className="anim-fadeup text-[12.5px] text-neutral-400 mb-8 max-w-sm" style={{ animationDelay: '90ms' }}>
                            ...
                            {STARTERS.map((s, i) => {
                              ...
                              return (
                                <button
                                  ...
                                  className="anim-fadeup group text-left p-4 rounded-xl border border-white/10 bg-white/[0.03] backdrop-blur-sm hover:border-amber-500/40 hover:bg-white/[0.06] hover:-translate-y-0.5 transition-all duration-300"
                                ...
                                >
                                  ...
                                  <span className="block text-[10px] uppercase tracking-[0.15em] text-neutral-400 mb-1">{s.label}</span>
                                  <span className="block text-[12.5px] text-neutral-200 leading-snug group-hover:text-white transition-colors">{s.prompt}</span>
                                </button>
                              );
                            })}
                          </div>
                                ...
                                <div
                                  className={`max-w-[82%] px-4 py-2.5 text-[14px] leading-relaxed whitespace-pre-wrap break-words ${
                                    m.role === 'user'
                                      ...
                                      : 'bg-white/[0.06] border border-white/10 text-neutral-100 rounded-2xl rounded-bl-md backdrop-blur-sm'
                                  }`}
                                >
                                  {m.content ? (
                                    m.content
                    ...
                    <div className="shrink-0 border-t border-white/10 p-3.5">
                      <div className="relative flex items-end w-full rounded-2xl border border-white/10 bg-white/[0.05] backdrop-blur-sm transition-all duration-300 focus-within:border-amber-500/60 focus-within:shadow-[0_0_0_4px_rgba(245,158,11,0.12)]">
                        <textarea
                          ref={inputRef}
                          rows={1}
                          value={textInput}
